...
//		header.setSeq(seq);
		...
		for( int j = 0; j < numBlocks; j++ ) {
			...
			for (int i = start; i < end; i++) {
				...
				String grName = players[i].getGroupName();
				long grId = players[i].getGid();
				 ...
				 */
				if (grId > 0) {
					data[idx] = Util.int2Word(players[i].getInvitation())[1];
				}
				...
				/*
				 * Despues del room_id podría venir el nivel del grupo 00 00 niv1 niv2 (4 bytes)
				 */
				System.arraycopy(Util.int2Word(0x3FFF), 0, data, idx+8, 2);
				System.arraycopy(Util.int2Word(players[i].getCategory()), 0, data, idx + 10, 2 );
				
				//System.arraycopy(Util.int2Word(1), 0, data, idx + 12, 2 );
				//Mentira es posible que el nivel del grupo vaya aqui!
				System.arraycopy(Util.int2Word(0x3FFF), 0, data, idx+12, 2);
