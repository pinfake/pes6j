...
//		header.setSeq(seq);
		...
		for( int k = 0; k < numBlocks; k++ ) {
			...
			for (int i = start; i < end; i++) {
				...
				/*
				 * Ni half ni ostias esto tiene que ser la fase!!! valgame dios
				 */
				byte[] b_half = Util.int2Word(rooms[i].getHalf());
				//byte[] b_phase = Util.int2Word(rooms[i].getGamePhase());
				//Puede haber un type especial para los kekitos? nu se.
				...
				for (int j = 0; j < rooms[i].getNumPlayers(); j++) {
					...
					// efectos extra�os
					// Bastante interesante deberemos mirarlo mas
					if (j == 0)
						data[idx + (10 * j) + 4] = (byte) 0x01;
			
					/*
					 * Cojones en +5 me falta un byte? hay un 01 para el lider también.
					 * No se que es, pero si le pongo un 01 a todos la jodemos bien. Podría ser
					 * el ready status?. Dwarf lo llama "random?".
					 */
					//data[idx + (10*j) + 5] = (byte)0x01;
	...
	//			System.arraycopy(b_last, 0, data, idx - 4, b_last.length);
				if( rooms[i].getMatchSettings() != null ) {
					/* 00 02 00 00 va perfecto sin cosas raras. */
					data[idx-4] = 0x00; // Primer byte a 1, a 02 y a 03 significa lo mismo también significa partido de grupos 
									    // A 0 es partido normal.
					data[idx-3] = 0x02; // Esto es si la gente puede chatear o no en el partido.
					data[idx-2] = 0x00; // No se lo que hace esto a 1 va bien. (pruebo con ceros)...
					data[idx-1] = 0x00; // No se lo que hace esto a 1 va bien. Aver 02? 00 01 y 02 parece que hagan lo mismo
					//	System.arraycopy(rooms[i].getMatchSettings(), 0, data, idx - 4, 4);
				}
