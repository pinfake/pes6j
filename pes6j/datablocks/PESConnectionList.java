...
public class PESConnectionList {
	...
	public synchronized void sendEncodedTo(long[] pids, Message mes) throws IOException {
	...
	public synchronized void sendEncodedToNIO(long[] pids, Message mes) throws IOException {
		for (int i = 0; i < pids.length; i++) {
			PESConnection c = searchConnection(pids[i]);
			if (c != null) {
				c.streamWriteEncodedNIO(mes);
